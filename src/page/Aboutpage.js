...
function AssetOrbit({ assets, totalAUM, aumDate }) {
  ...
  const subReach = hasSubs
    ? maxBubbleHalf + SUB_ORBIT_OFFSET + SUB_SIZE / 2
    : maxBubbleHalf;
          ...
          >
            <div
              ...
              style={{
                ...
                marginLeft: -MAIN_R,
                marginTop: -MAIN_R,
                border: "1px dashed rgba(196,160,120,0.3)",
              }}
            />
            <div
              className="ring"
              style={{
                width: (MAIN_R + 28) * 2,
                height: (MAIN_R + 28) * 2,
                marginLeft: -(MAIN_R + 28),
                marginTop: -(MAIN_R + 28),
                border: "0.5px dotted rgba(196,160,120,0.1)",
              }}
            />
            {assets.map((asset, i) => {
            ...
            >
              ...
              >
                {activeAsset ? "SELECTED" : "TOTAL PORTFOLIO"}
              </span>
              <span
                style={{
                  fontSize: activeAsset ? 16 : 20,
                  fontWeight: 700,
                  color: "#000",
                  lineHeight: 1,
              ...
              >
                {activeAsset ? activeAsset.pct : totalAUM}
              </span>
              <span
                style={{
                  fontSize: 9,
              ...
              >
                {activeAsset ? activeAsset.value : aumDate || "Dec 31, 2025"}
              </span>
            </div>
          </div>
        </div>
...
export default function AboutPage() {
  ...
  const whoWeAre = {
    para1:
      "NF Holdings is a pan-African family-owned investment holding company with investment in various sectors in East Africa.",
    para2:
      "NF Holdings is inspired by our mission to create a legacy, for all Africans who will inherit the Africa we are building today. We create, grow and preserve value for our stakeholders – while driving Africa's sustainable economic and social development.",
  };
  ...
  const staticValues = [
    ...
    {
      label: "Impact",
      body: "We champion long-term continental progress over short-term gains. We, make deliberate, high impact decisions that drive measurable social and economic contributions, leaving a lasting, positive legacy for all Africans.",
    },
  ];
  ...
  useEffect(() => {
    ...
    const fetchAboutData = async () => {
      try {
        ...
        if (
          Array.isArray(aboutData.timeline) &&
    ...
    const fetchSiteStats = async () => {
      try {
        ...
        setStats((prev) => {
          const currentAUM =
            prev.find((s) => s.label === "TOTAL PORTFOLIO")?.value ||
            siteData.totalPortfolioValue?.displayText ||
            "N/A";
          const countries =
            siteData.totalCountries != null
              ? String(siteData.totalCountries)
              : "N/A";
          const companies =
            siteData.totalCompanies != null
              ? String(siteData.totalCompanies)
              : "N/A";
          const clusters =
            siteData.totalClusters != null
              ? String(siteData.totalClusters)
              : "N/A";
    ...
    >
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (min-width: 700px) { .orbit-section { flex-direction: row; align-items: center; justify-content: flex-start; gap: 6px; } }
...
@media (min-width: 1024px) {
  .val-grid {
    ...
    gap: 16px;
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(to right, rgba(0,0,0,0.62) 0%, rgba(0,0,0,0.38) 60%, rgba(0,0,0,0.55) 100%)",
          }}
        />
        <div
          className="pg"
      ...
      <div className="pg">
        <div className="col">
          <div className="sec" style={{ paddingBottom: 4 }}>
            <div style={{ marginBottom: "clamp(6px, 1.2vw, 14px)" }}>
              ...
              >
                As at {aumDate || "July 30, 2026"} · TOTAL PORTFOLIO {totalAUM}{" "}
                · Sub-entities orbit around each parent asset
              </p>
            </div>
              ...
              {assets.length > 0 ? (
                ...
                <div className="breakdown-skeleton">
                  {[1, 2, 3, 4].map((item) => (
                    <div
                      key={item}
                      className="skeleton breakdown-skeleton-card"
                    />
                  ))}
                </div>
              )}
