...
const CompanyDetailPage = () => {
  ...
  // Find the partnership stub from sectorsData
  let company = null;
  ...
  // Full detail record from companyDetails
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Link to="/clusters" className="text-[#0A2540] hover:text-[#1C1F26] font-medium">Return to Portfolio</Link>
        </div>
      </div>
    );
  }
  ...
  const barChartOptions = {
    ...
    scales: {
      x: {
        grid: { display: false },
        ticks: { color: '#e2e8f0', font: { size: 12, family: "'Inter', sans-serif" }, maxRotation: 0 },
        border: { display: false },
      },
      y: {
        grid: { color: 'rgba(255,255,255,0.08)' },
        ticks: { color: '#e2e8f0', font: { size: 13, family: "'Inter', sans-serif" }, callback: v => v.toLocaleString() },
        border: { display: false },
        beginAtZero: true,
      },
    },
  ...
  // ── Separator helper ─────────────────────────────────────────
  const Sep = () => <div className="text-[#1C1F26]">|</div>;
        ...
        <div className="grid w-full grid-cols-12 mx-auto max-w-screen-3xl">
          ...
          <div className="col-span-12 lg:col-span-10">
            <div className="px-4 sm:px-6 lg:px-8">
              <Link to="/clusters" className="inline-flex items-center text-[#0A2540] hover:text-[#1C1F26]">
        ...
        <div className="col-span-12 lg:col-span-10">
          <div className="px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="mb-8">
              ...
              <h1 className="mb-4 text-5xl font-bold text-[#0A2540]">{detail.name}</h1>
              <div className="flex flex-wrap items-center gap-4 text-base text-[#1C1F26]">
                <div className="font-semibold text-[#0A2540]">
                  {detail.KESValue} <span className="text-sm text-[#1C1F26]">KES</span>
                </div>
                <Sep />
                <div className="font-semibold text-[#0A2540]">
                  {detail.usdValue} <span className="text-sm text-[#1C1F26]">USD</span>
                </div>
              </div>
            </div>
